...
struct lock {
	char *name;
	volatile int lock_occupied; // 0 if lock not occupied, 1 if lock is occupied
	volatile struct thread *holder; // This thread will represent the thread holding the lock
